...
public class Quiz2206 {
	...
	public static void main(String[] args) throws IOException {
		...
		bfs();
		if(visit[N-1][M-1][0] == 0 && visit[N-1][M-1][1] == 0) System.out.println(-1);
		else if(visit[N-1][M-1][0] == 0) System.out.println(visit[N-1][M-1][1]);
		else if(visit[N-1][M-1][1] == 0) System.out.println(visit[N-1][M-1][0]);
		else System.out.println(Math.min(visit[N-1][M-1][0], visit[N-1][M-1][1]));
	}
	private static void bfs() {
		...
		while(!ad.isEmpty()) {
			...
			for(int i=0; i<4; i++) {
				...
				if(arr[mr][mc] == 1 && breakCount==1) continue;
				if(arr[mr][mc] == 1 && breakCount==0) newBreakCount++;
